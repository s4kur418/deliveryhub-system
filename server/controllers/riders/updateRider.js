...
export const updateRider = (req, res) => {
  ...
  const sql =
    "UPDATE riders SET name=?, email=?, phone=?, vehicleType=?, vehicleNumber=?, status=? WHERE id=?";

  db.query(sql, [name, email, phone, vehicleType, vehicleNumber, status || "available", id], (err) => {
